...
uvx "$RUFF" check atlas fleet notify --select F,B,E722
...
set +e
bandit_out="$(uvx "$BANDIT" -q -r atlas fleet notify --severity-level medium 2>&1)"
bandit_rc=$?
set -e
...
uvx "$MYPY" --python-version 3.11 --allow-redefinition --exclude 'fleet/check_fleet\.py|notify/check_notify\.py' atlas fleet notify
