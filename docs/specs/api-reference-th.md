...
| GET | `/api/jobs/{job_id}/artifacts` | ไฟล์ที่เก็บของ job (T5 `file_ref` artifacts) |
...
### การส่งไฟล์ระหว่าง node (`push_files`, T6)

edge สามารถ push ไฟล์ที่เก็บไว้ก่อนหน้า (ดู `collect_files`, T5) เข้าไปใน workspace
ของ worker **ตัวถัดไป** ก่อนที่ job ของ node ปลายทางจะเริ่ม — เพื่อให้สาย
...
- **เพิ่มอย่างเดียว (additive)** ไฟล์ลงใต้ `incoming/<run_id>/<node_key>/…` ใน worker
  ปลายทาง Atlas ไม่เคยเรียก trash/replace/delete จึง push ทับไฟล์เดิมของปลายทางไม่ได้
  โทเคน `{files_dir}` ใน prompt ปลายทางจะถูกแทนด้วย prefix `incoming` นั้น
- ใช้เพดาน sync ของ T5 ซ้ำ (`ATLAS_SYNC_MAX_FILES`/`ATLAS_SYNC_MAX_BYTES`) และ
  `sync_mode` gate ของ worker ปลายทาง (ปลายทางที่ปิดอยู่จะทำให้ edge ล้ม) tar ขาออกมี
  ลำดับ member และ mtime ที่ normalize (bytes ทำซ้ำได้สำหรับ audit `files.pushed`)
  `409 workspace busy` จะ retry ส่วนความล้มเหลวอื่นของ push จะทำให้ edge ล้มชัดเจน
