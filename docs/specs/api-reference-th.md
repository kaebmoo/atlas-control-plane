...
สถานะ: **Current API specification v1.2**<br>
...
`import` ต้องมีสิทธิ์ `workflows.manage`; ส่วนการอ่านต้องมี `read`.
...
`local` API ยังไม่มี filter/cursor และไม่มี endpoint ลบ audit
...
[openapi.yaml](openapi.yaml) ระบุ 51 paths และ 70 operations พร้อม security schemes,
...
## 15. Checklist สำหรับ API client
