...
| **Worker-supplied files (T5): `POST /workspace/sync/export` responses** | Semi-trusted inbound tar — a compromised/buggy worker can return a hostile gzip archive | The sync surface is **not Bearer-authed** (contract doc), so it is only ever called on a worker whose operator asserted an approved shape via the T4 `sync_mode` gate (`tunnel`/`forward_auth`); a `disabled` worker is never contacted (`files.collection_skipped`). Every ingested tar goes through **one shared validator** (`atlas/sync_files.py::safe_extract_tar`) — the single reader, so no laxer path exists: it **rejects absolute paths, `..` traversal, backslash-smuggled separators, and any non-regular member** (symlink/hardlink/device/fifo — the escape vectors), and enforces the **file-count (`ATLAS_SYNC_MAX_FILES`) and total-uncompressed-byte (`ATLAS_SYNC_MAX_BYTES`) caps while streaming**, so a decompression bomb aborts before its bytes are realized. Extracted bytes are written **only under a fresh opaque id** in the upload store (`store_bytes`), never a path derived from a member name, so even a filter miss cannot escape the store. The collection call is bounded in size (`max_bytes`) and **wall-clock (`ATLAS_COLLECT_DEADLINE_SECONDS`)** and runs as a **pre-terminal barrier that is failure-isolated** — any failure records `files.collection_failed` (audited with **counts only, never contents**) and the job still reaches `succeeded`, so a hostile or hung worker can neither wedge a job non-terminal nor change its outcome. The request path list is itself validated relative + `..`-free before dispatch. Enforced by `scripts/check_file_collection.py` (mutation-locked: disabling the `..` rejection, moving the barrier past the succeeded write, or probing a disabled worker all turn the gate red). |
| **File handoff to workers (T6): `POST /workspace/sync/push`** | Atlas WRITES to a semi-trusted worker's workspace | **Additive-only, by construction.** Atlas builds every tar arcname as `incoming/<run_id>/<node_key>/…` (`build_push_tar`, deterministic + normalized for reproducible audit bytes) and **never sends any replace/trash/delete option**, so a push can only add files under an opaque per-run prefix and can never clobber the target worker's own files. **Opt-in only:** an edge `push_files` requires `policy.file_handoff` — enforced at graph-save time AND re-checked as a runtime guard in `_submit_worker_node` (a push cannot happen without the policy even if validation were bypassed). Same sync gate as T5: the target worker's operator-asserted `sync_mode` must be an approved shape (a `disabled` target fails the edge loudly, never a silent skip). Reuses the T5 caps (`ATLAS_SYNC_MAX_FILES`/`ATLAS_SYNC_MAX_BYTES`, checked on the outgoing set by ACTUAL byte length). **The source `file_ref` artifacts are NOT assumed pre-validated:** `POST /api/artifacts` lets any authenticated caller create a `file_ref` with an arbitrary `content` path and `relpath`, so `_push_files_to_worker` RE-VALIDATES every selected artifact before use — the same containment `_download_artifact` enforces (`(upload_dir/content).resolve().parent == upload_dir.resolve()`, so a `../…`/absolute `content` can't exfiltrate a host file) plus `_reject_unsafe_path` on the relpath (so it can't escape the `incoming/` prefix); a violation fails the edge. Deadline-bounded with a bounded 409-retry; the network push runs OUTSIDE the shared workflow-runner lock (mirroring the unlocked job wait) so it can't stall other runs' stepping or operator pause/cancel. A failed push raises and fails the edge (no partial-success handoff); `files.pushed` is audited (count/bytes/sha256/target) only after a successful push and before the downstream job is created. Enforced by `scripts/check_file_handoff.py` (two mock workers; mutation-locked: dropping the runtime file_handoff guard makes a no-policy push reach the worker; removing the save-time cross-check stops rejecting push_files without the policy; dropping the upload-store containment check lets a crafted artifact exfiltrate an out-of-store file; the mock asserts trash/replace are never called). |
