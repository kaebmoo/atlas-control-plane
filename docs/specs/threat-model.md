...
|---|---|---|
| **HTTP API callers** | Authenticated + RBAC | per-user **API token = SHA-256 of a 256-bit random token** (high-entropy, so no KDF needed); **passwords = PBKDF2-HMAC-SHA256, 600k iters**; dashboard sessions have immutable `purpose=session`, 8-hour default expiry, expiry enforcement on header and SSE query auth, and a five-session cap that revokes only oldest excess sessions; admin-issued `purpose=api` tokens may be explicitly non-expiring or carry a future expiry; login is pre-PBKDF2 rate-limited by normalized username + direct TCP peer IP (bounded in-memory 5/min + 60-second cooldown, `429`/`Retry-After`, intentionally no `X-Forwarded-For` trust, and process restart resets the short window); role→permission map; `?token=` only on SSE streams; secrets never returned or audited |
...
| **Pack files** | Semi-trusted | validated through the shared engine validators; signature optional, enforceable via `ATLAS_REQUIRE_SIGNED_PACKS`. Since D2c-2 a pack's `policy` may carry `approval_webhook_url` — an outbound destination — so an imported pack can name where reminders go. Shape-validated at import and re-resolved against `ATLAS_OUTBOUND_ALLOWLIST` at send time, so an imported URL can only reach a host the operator already allows |
...
Not fixed because the model makes them unreachable / out-of-threat — except item 5, which was found to be a live defect and fixed in D2c-1; it is kept in place for the audit trail. Each: rationale → trigger that
turns it into real work. **Owner of all six remaining accepted risks: Pornthep Nivatyakul** (named sign-off,
...
2. **Independent review** — a separate reviewer, given the claims but **not** the self-check,
   ...
   check, not a parsed scheme; and of the (twelve at the time of this review; seventeen as of migration 017) migration steps, `SCHEMA` is the only bare
