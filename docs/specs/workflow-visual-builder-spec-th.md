...
- root ต้องมี `name`, `graph`, `policy`; `description` และ `default_reply` เป็น optional
- เมื่อมี `default_reply` ให้ใช้รูปแบบ `input._meta.reply` จะ copy ไป run ใหม่เฉพาะเมื่อ run
  นั้นไม่ส่ง `_meta.reply`; `webhook` ต้องมี `callback_url` ที่ไม่ว่างและอยู่ใน allowlist ส่วน
  `none` ละได้ Inspector ต้องล้างค่านี้ได้
