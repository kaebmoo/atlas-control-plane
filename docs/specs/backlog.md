...
| L2 | **No static type-checking in the toolchain** — `scripts/lint.sh` runs ruff + bandit only; there is no `mypy` config. Annotations exist but are unverified. | Not a bug; runtime behavior is covered by the hermetic gate. | Pornthep Nivatyakul | A type-confusion bug ships, or before opening the repo to outside contributors → add `mypy` to `lint.sh`. |
