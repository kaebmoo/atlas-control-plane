...
| L3 | **Bandit gates at `--severity-level medium`** — Low-severity findings are not enforced. The reviewed suppressions are **B608** (8 `db._set_clause` UPDATEs + 2 dynamic-SELECT statements — column names from a fixed allowlist, values parameterized) and **B310** (5 `urlopen` calls — guarded by http(s)-only `base_url`), each carrying a per-line `# nosec <code>` with rationale. | Suppressions are injection-/SSRF-safe by construction and reviewed. (NB: there is **no B105** in the tree — an earlier note citing "B105 false positives" was inaccurate.) | Pornthep Nivatyakul | Re-confirm each `# nosec` stays justified whenever its surrounding code changes. |
