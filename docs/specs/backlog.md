...
|---|---|---|---|---|
| L1 | ~~**Mobile sidebar usability** — the dashboard sidebar (`atlas/static/`) is awkward on narrow viewports.~~ **Closed 2026-07-01** — sidebar collapses to a hamburger + drawer nav below 860px (commit `0dddaa0`), gate-enforced by `scripts/check_ui_ux.py`. | UX only; no security, data, or correctness impact. | Pornthep Nivatyakul | Closed — reopen only if a regression removes the hamburger/drawer behavior. |
| L3 | **Bandit gates at `--severity-level medium`** — Low-severity findings are not enforced. The reviewed suppressions are **B608** (14 in `atlas/db.py` — `db._set_clause`-built UPDATEs and dynamic SELECTs; column names from a fixed allowlist, values parameterized) and **B310** (2 in `atlas/thclaws_client.py` — guarded by http(s)-only `base_url`), each carrying a per-line `# nosec <code>` with rationale. | Suppressions are injection-/SSRF-safe by construction and reviewed. (NB: there is **no B105** in the tree — an earlier note citing "B105 false positives" was inaccurate.) | Pornthep Nivatyakul | Re-confirm each `# nosec` stays justified whenever its surrounding code changes. |
