...
รายละเอียดของ run อาจขึ้นแถบเหลืองบอกว่า workflow นี้ตั้งค่าบางอย่างที่ระบบรับไว้แต่
ไม่ได้ใช้จริง เช่น `collect_files` บนโหนดที่ไม่ได้รัน job จึงไม่มีการเก็บไฟล์ตรงนั้นเลย
ตัว run ไม่ได้รับผลกระทบ ถ้าตั้งใจให้เก็บไฟล์ ให้ย้ายค่านั้นไปไว้ที่โหนด `worker`/`manager`
