...
| `429` | ถึง login rate limit; รอตามระยะ `Retry-After` |
...
token นี้เป็น `purpose=session` ไม่ใช่ integration credential ทั่วไป: หมดอายุปริยายใน 8 ชั่วโมง
และ `GET /api/me` คืน `session.expires_at` ซ้ำเพื่อให้ UI เตือนหรือ sign out ได้อย่างถูกต้อง
ผู้ใช้หนึ่งคนมี session ที่ยังไม่หมดอายุได้ไม่เกิน 5 รายการ; login ใหม่จะ revoke เฉพาะรายการที่เกิน
และเก่าที่สุด `POST /api/auth/logout` จะ revoke token ที่ใช้อยู่

การ login ที่ล้มเหลวถูกจำกัดก่อน PBKDF2 ด้วย normalized username + direct peer IP (ปริยาย
5 ครั้ง/นาที แล้ว cooldown 60 วินาที) หากเกินจะตอบ `429` พร้อม `Retry-After`; client ควรรอแทน
การ retry ตัว limiter นี้เป็นชั้นป้องกันในหน่วยความจำและถูกล้างเมื่อ Atlas restart จึงควรมี rate
limit ที่ reverse proxy/WAF ด้วย Atlas ไม่เชื่อ `X-Forwarded-For` โดยเจตนา
...
machine token มี `purpose: "api"` ซึ่งแก้ไม่ได้; ไม่ส่ง `expires_at` เฉพาะเมื่อ integration
credential ตั้งใจให้ไม่หมดอายุ metadata ของ list/get token แสดง `purpose`, `expires_at` และ
สถานะ revoke แต่ไม่แสดง raw token หรือ hash
