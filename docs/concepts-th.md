...
edge ยังใส่ `push_files` ได้: รายการ glob ของ artifact key ที่ส่งไฟล์ที่เก็บไว้แล้ว
(`collect_files`, [§5](#5-ชนิด-node)) ให้ worker ปลายทางก่อน job ถัดไปเริ่ม ต้องเปิด
`policy.file_handoff` ก่อน ([§10](#10-นโยบายและ-guard)); ดู
[API reference](specs/api-reference-th.md) (T9b)

---

## 8. ตัวแปรใน prompt

prompt ของ worker/manager แทนค่า `{...}` จากหลายราก:

| ตัวแปร | แหล่งข้อมูล |
| --- | --- |
| `{input.X}` | JSON ของ run input |
| `{artifact.KEY}` | เนื้อหา artifact ตามคีย์ |
| `{run.KEY}` *(ขั้นสูง)* | ข้อมูล metadata ของ run |
| `{node.KEY}` *(ขั้นสูง)* | field ของ node ปัจจุบันเอง |
...
- worker ปลายทางที่รับผ่าน edge `push_files` ([§7](#7-เงื่อนไข-edge)) ยังเห็นตัวแปรพิเศษ
  ที่ไม่มี dot คือ `{files_dir}` — ถูกแทนด้วย path ของไฟล์ที่ส่งมา หรือว่างถ้าไม่มีการ push
...
artifact ยังใส่ป้ายชั้นความลับของข้อมูล (`public`, `internal`, `confidential`,
`secret`) แบบไม่บังคับได้ เก็บไว้ที่ `metadata.classification` — ดู
...
| `file_handoff` | ต้องเปิดก่อน edge จะใส่ `push_files` ได้ ([§7](#7-เงื่อนไข-edge)); ค่าเริ่มต้นปิด |
...
- `tokens_prompt`/`tokens_output` ต่อ job เก็บจาก `usage` event ของ worker
  (thClaws v0.85.0 ขึ้นไป) และรวมเข้ายอด period totals จะเป็น NULL สำหรับ worker
  ที่ไม่ส่ง usage; model/token มีไว้เพื่อ visibility เท่านั้นภายใต้ BYOK ไม่นำไปคิดเงิน
  (`byok_token_counts_billable` ยังเป็น false)
