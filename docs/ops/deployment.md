...
| `ATLAS_OUTBOUND_ALLOWLIST` | — (empty) | Comma-separated hostnames/CIDRs Atlas may POST to. **Empty disables outbound delivery entirely** (secure default): every delivery is recorded `blocked` and no socket is opened. Also gates a workflow's own `policy.approval_webhook_url`, so an author can only address hosts you allow. |
| `ATLAS_OUTBOUND_MAX_ATTEMPTS` | `5` | Attempts before a delivery is dead-lettered as `failed`. |
| `ATLAS_OUTBOUND_TIMEOUT` | `10` | Seconds per attempt — total wall clock covering connect, request and response headers, not a per-recv timeout. |
| `ATLAS_APPROVAL_WEBHOOK_URL` | — | Deployment default for approval-overdue reminders; each workflow may override it with `policy.approval_webhook_url`. With no URL configured anywhere the sweep is inert. Must point at a receiver you run — Atlas only POSTs a signed body, it sends no email or chat ([`poc/approval_reminder_receiver.py`](../../poc/approval_reminder_receiver.py) is a runnable reference). The body is contract v1 — additive-only; a breaking change would arrive as a new event name (see the API reference §14). |
| `ATLAS_APPROVAL_OVERDUE_HOURS` | — | Deployment default escalation ladder, e.g. `72,168`. Ascending; the position is the level carried in the delivery body. Wall-clock hours — business days and holidays are the receiver's to apply, so leave room for a weekend. |
